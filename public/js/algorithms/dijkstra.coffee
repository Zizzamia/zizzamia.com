...
var Q = clone(node);  // Q è una coda di vertici di min-priorità, implentata o con un heap-binario o con un heap di Fibonacci
...
function setGraph() {
	...
	node[0] = new Array('s', 0, 'nil')
	node[1] = new Array('t', 0, 'nil')
	node[2] = new Array('y', 0, 'nil')
	node[3] = new Array('x', 0, 'nil')
	node[4] = new Array('z', 0, 'nil')

	/* archi orientati */
	node[0][3] = new Array('t','y')
	node[1][3] = new Array('x','y')
	node[2][3] = new Array('t','x','z')
	node[3][3] = new Array('z')
	node[4][3] = new Array('s','x')
...
function dijkstra() {
	...
	if (u) {	
		var i = nodeNumberTwo(node, u, 0)			
		Min = Number.MAX_VALUE		
		
		for (var j = 0, co = node[i][3].length; j < co; j++) {
			var z = nodeNumberThreeTwo(node, node, i, 3, j)
	...
	if (canvas.getContext){  // Make sure we don't execute when canvas isn't supported
		...
		for (indice = 0; indice < node.length; indice++) {
			for (k = 0, c = node[indice][3].length; k < c; k++) {	
				var z = nodeNumberThreeTwo(node, node, indice, 3, k) // recupero il numero dell'indice analizzato	
			
				posX = nodeCp1x (indice, z, node[indice][6][k])
				posY = nodeCp1y (indice, z, node[indice][6][k])
				if (node[indice][0] == node[z][2]) color = '#7FB24C'
				else color = '#ddd'
